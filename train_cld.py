...
from utils import maybe_mkdir, get_lr, fetch_data, seed_worker, poly_lr, sliding_window_inference, EMA
...
parser.add_argument('--exp', type=str, default='cld') # 实验名称
...
# --- ConfidenceBank-CPS 适配: 添加新参数 ---
parser.add_argument('--conf_momentum', type=float, default=0.99, help='Momentum for ConfidenceBank EMA update')
parser.add_argument('--conf_update_iter', type=int, default=8, help='Update period for ConfidenceBank')
parser.add_argument('--sam_gamma', type=float, default=0.5, help='Gamma for calculating sampling rate')
# --- End ConfidenceBank-CPS 适配 ---
...
# --- 辅助函数区 ---
# ... (dice_from_labels_gpu, ModelEnsemble, kaiming/xavier_init, sigmoid_rampup, get_current_consistency_weight 与 baseline 完全相同, 为简洁省略)
...
# --- ConfidenceBank-CPS 适配: 定义支持掩码的损失函数 ---
class MaskedRobustCrossEntropyLoss(RobustCrossEntropyLoss):
    def forward(self, input, target, mask=None):
        # target in shape [B, 1, D, H, W]
        # mask in shape [B, D, H, W]
        target = target.squeeze(1).long()
        loss = F.cross_entropy(input, target, reduction='none')
        if mask is not None:
            # Flatten all spatial dimensions for easier calculation
...
def make_loss_function(name, weight=None):
    if name == 'sup': return DC_and_CE_loss(n_classes=config.num_cls, w_dc=weight, w_ce=weight)
    elif name == 'cps': return MaskedRobustCrossEntropyLoss(weight=weight)
    else: raise ValueError(name)

# --- ConfidenceBank-CPS 适配: 核心算法类和函数 ---
class ConfidenceBank:
    def __init__(self, num_cls, momentum, update_iter):
        self.num_cls = num_cls
        self.momentum = momentum
        self.update_iter = update_iter
        self.tot_iter = 0
        self.confs = torch.rand(self.num_cls, dtype=torch.float32).cuda()
        self.confs_all = torch.zeros(self.num_cls, dtype=torch.float32).cuda()
        self.confs_cnt = torch.zeros(self.num_cls, dtype=torch.float32).cuda()
    ...
    def record_data(self, output, label):
        ...
        for ind in range(self.num_cls):
            ...
            self.confs_all[ind] += torch.sum(conf_map_sup * gt_mask)
            self.confs_cnt[ind] += gt_mask.sum()
        
        self.tot_iter += 1
        # Periodically update the confidence scores
        if self.tot_iter > 0 and self.tot_iter % self.update_iter == 0:
            new_confs = self.confs_all / (self.confs_cnt + 1e-12)
            self.confs = EMA(new_confs, self.confs, momentum=self.momentum)
            # Reset accumulators
            self.confs_all.zero_()
            self.confs_cnt.zero_()
            # logging.info(f"ConfidenceBank updated.")

    def get_confs(self):
        return self.confs

def cal_sampling_rate(confs, gamma):
    sam_rate = 1.0 - confs
    sam_rate = sam_rate / (torch.max(sam_rate) + 1e-12)
    sam_rate = torch.pow(sam_rate, gamma)
    return sam_rate

def cal_sampling_mask(output, sam_rate):
    pred_map = torch.argmax(output.detach(), dim=1)
    sample_map = torch.zeros_like(pred_map, dtype=torch.float32)
    
    for idx in range(config.num_cls):
        if sam_rate[idx] < 1.0: # No need to sample if rate is 1 (confidence=0)
            prob_to_keep = 1.0 - sam_rate[idx]
            class_mask = (pred_map == idx)
            rand_map = torch.rand_like(pred_map, dtype=torch.float32)
            
            selected_pixels = (rand_map < prob_to_keep) & class_mask
            sample_map[selected_pixels] = 1.0
...
if __name__ == '__main__':
    ...
    sup_loss_func = make_loss_function('sup')
    cps_loss_func = make_loss_function('cps')
    
    # --- ConfidenceBank-CPS 适配: 初始化 ConfidenceBank ---
    confs_A = ConfidenceBank(num_cls=config.num_cls, momentum=args.conf_momentum, update_iter=args.conf_update_iter)
    confs_B = ConfidenceBank(num_cls=config.num_cls, momentum=args.conf_momentum, update_iter=args.conf_update_iter)
    # --- End ConfidenceBank-CPS 适配 ---
    ...
    for epoch_num in range(args.max_epoch + 1):
        ...
        for batch_l, batch_u in tqdm(zip(labeled_loader, unlabeled_loader), total=len(unlabeled_loader), desc=f"epoch {epoch_num} training"):
            ...
            with autocast('cuda'):
                ...
                # --- ConfidenceBank-CPS 适配: 核心逻辑 ---
                # 1. 累积有标签数据的置信度
                confs_A.record_data(output_A_l, label_l)
                confs_B.record_data(output_B_l, label_l)
                
                # 2. 获取当前置信度并计算采样率
                current_confs_A = confs_A.get_confs()
                current_confs_B = confs_B.get_confs()
                sam_rate_A = cal_sampling_rate(current_confs_A, gamma=args.sam_gamma)
                sam_rate_B = cal_sampling_rate(current_confs_B, gamma=args.sam_gamma)
                ...
                # 使用带掩码的损失函数
                loss_cps = cps_loss_func(output_A, pseudo_B, sample_map_A) + cps_loss_func(output_B, pseudo_A, sample_map_B)
            ...
            loss_list.append(loss.item()); loss_sup_list.append(loss_sup.item()); loss_cps_list.append(loss_cps.item())
        ...
        writer.add_scalars('sampling_rate/A', {f'Class_{i}': r for i, r in enumerate(sam_rate_A.cpu().numpy())}, epoch_num)
        writer.add_scalars('sampling_rate/B', {f'Class_{i}': r for i, r in enumerate(sam_rate_B.cpu().numpy())}, epoch_num)

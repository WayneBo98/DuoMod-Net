...
class FeCLoss(nn.Module):
    ...
    def forward(self, feat, mask, teacher_feat=None, gambling_uncertainty=None, epoch=0):
        ...
        if self.use_focal:
            ...
            focal_weights[hard_pos_mask] = (1.0 - similarity[hard_pos_mask]).clamp(min=1e-6).pow(self.gamma)
        ...
        if teacher_feat is not None:
            ...
            if cross_hard_neg_mask.sum() > 0:
                ...
                safe_cross_sim = cross_sim.clamp(max=0.999999) # 防止 cross_sim 精确等于 1
                loss_cross_term = -torch.log(1 - safe_cross_sim + 1e-18)

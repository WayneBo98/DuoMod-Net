...
#     epsilon = 1e-4  # 使用一个对 float16 安全的 epsilon

#     # --- 终极修复：在使用前将输入裁剪到安全范围 ---
#     pred = torch.clamp(pred, min=epsilon, max=1.0 - epsilon)
#     res = torch.clamp(res, min=epsilon, max=1.0 - epsilon)
    
#     # --- 修复后，log 内部不再需要手动加 epsilon ---
#     loss1 = - res * torch.log(pred) - (1 - res) * torch.log(1 - pred)
#     loss1 = loss1/2
#     # 分母的 epsilon 仍然需要，以防 cls_label 全是0
#     loss1 = (loss1*cls_label).sum(1)/(cls_label.sum(1) + epsilon) 
#     loss1 = loss1.mean()

#     true_mask_one_hot = one_hot(true_mask, k)
    
#     loss2 = - true_mask_one_hot * torch.log(res) \
#             - (1 - true_mask_one_hot) * torch.log(1 - res)
#     loss2 = loss2/2
#     loss2 = (loss2 * cls_label).sum(1) / (cls_label.sum(1) + epsilon)

#     loss2_filtered = loss2[true_mask < k]
#     if loss2_filtered.numel() > 0:
#         loss2 = loss2_filtered.mean()
#     else:
...
def cal_gmm_loss(pred, res, cls_label, true_mask):
    ...
    true_mask = true_mask.cuda()
    epsilon = 1e-4
    loss1 = - res * torch.log(pred + epsilon) - (1 - res) * torch.log(1 - pred + epsilon)
    loss1 = loss1/2
    loss1 = (loss1*cls_label).sum(1)/(cls_label.sum(1)+epsilon)
    loss1 = loss1.mean()
    ...
    loss2 = - true_mask_one_hot * torch.log(res + epsilon) \
            - (1 - true_mask_one_hot) * torch.log(1 - res + epsilon)
    loss2 = loss2/2
    loss2 = (loss2 * cls_label).sum(1) / (cls_label.sum(1) + epsilon)

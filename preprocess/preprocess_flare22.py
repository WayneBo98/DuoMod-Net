...
def process(base_dir,save_dir_npy,save_dir_nii,target_spacing=(1.5, 1.5, 2.0)):
    ...
    # for tag in ['Tr']:
        ...
        for path in tqdm(glob.glob(os.path.join(base_dir, f'images{tag}', '*.nii.gz'))):
            ...
            label_path =os.path.join(base_dir, f'labels{tag}', f'{img_id[:-5]}.nii.gz')
            
            resample_image,resample_label,image, label = resample_to_spacing(image_path, label_path, target_spacing=target_spacing)
            sitk.WriteImage(resample_image, os.path.join(save_dir_nii,f'images{tag}', f'{img_id[:-5]}.nii.gz'))
            sitk.WriteImage(resample_label, os.path.join(save_dir_nii,f'labels{tag}', f'{img_id[:-5]}.nii.gz'))
            np.save(os.path.join(save_dir_npy,f'images{tag}', f'{img_id[:-5]}_image.npy'),image)
            np.save(os.path.join(save_dir_npy,f'labels{tag}', f'{img_id[:-5]}_label.npy'),label)
...
if __name__ == '__main__':
    base_dir = '/data/wangbo/CissMOS/Datasets/FLARE2022'
    save_dir_npy = '/data/wangbo/CissMOS/Datasets/FLARE2022_1.5_2.0_npy'
    save_dir_nii = '/data/wangbo/CissMOS/Datasets/FLARE2022_1.5_2.0_nii'
